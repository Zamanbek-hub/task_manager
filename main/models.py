...
class Column(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('del_column', kwargs={'column_id': self.pk,
                                             'board_id': self.board.pk})  # ВОТ ТУТ ПЫТАЛСЯ МЕНЯТЬ
...
class Card(models.Model):
    ...
    deadline = models.DateField(verbose_name="Срок выполнения", null=True,
                                blank=True)
    photo = models.ImageField(upload_to="photos/", verbose_name="Фото",
                              null=True, blank=True)
    column = models.ForeignKey(Column, on_delete=models.CASCADE, editable=False,
                               verbose_name="В какой колонке")
